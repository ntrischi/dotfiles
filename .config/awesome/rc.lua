...
local lain      = require("lain") -- {{ utils and widget }}
-- }}}
...
-- {{{ Tags
tags = {
   names = { "terms", "www", "code", "docs", "music", "mail" },
   layout = { layouts[1], layouts[3], layouts[3], layouts[3], layouts[3], layouts[3] }
}
...
yawn = lain.widgets.yawn(2422673, {
    settings = function()
        widget:set_markup(markup("#FFFFFF", forecast:lower() .. " @ " .. units .. "°F "))
    end
})
...
--{{{ Start a window terminal on startup
run_once("urxvt -g 100x26+350+250 -depth 32")
